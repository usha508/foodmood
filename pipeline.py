...
MOOD_LABELS = {
    0: ("Energy Crash",     "⚡💥", "#ff9800"),
    2: ("Sluggish",         "😴🌀", "#9c27b0"),
    3: ("Balanced & Alert", "😊⚖️", "#4caf50"),
}
...
def get_nutrition_for_meal(meal_text):
    ...
    total = {"calories":0,"protein":0,"fat":0,"carbs":0,"sugar":0,"fiber":0}
    for food in foods:
...
def engineer_features(nutrition):
    ...
    return {
        ...
        "balance_score":       1 - (
            ...
            abs((nutrition["fat"]*9/cal)     - 0.30)) / 2,
...
def predict_mood(meal_text, model, feature_cols):
    ...
    prot_pct  = (nutrition["protein"] * 4) / cal

    if nutrition["sugar"] > 35 or sugar_pct > 0.40:
        pred = 0
    elif fat_pct > 0.60 and cal > 400:
        pred = 2
    elif prot_pct > 0.40 and nutrition["sugar"] < 10:
        pred = 3
    else:
        X    = pd.DataFrame([features])[feature_cols]
        pred = int(model.predict(X)[0])

    label, emoji, color = MOOD_LABELS.get(pred, ("Balanced","😊","#4caf50"))
    probs = model.predict_proba(pd.DataFrame([features])[feature_cols])[0]
